...
from ia_mistral.ai_session import AISession
...
def on_node_change(event: MessageData, client: PocketBase):
    ...
    # Vérifie si la session est déjà complétée
    if getattr(session, 'completed', False):
        utils.log_message(f"Activité détectée mais session '{getattr(session, 'name', 'Unknown')}' déjà complétée (ID du node: {getattr(session, 'id', None)})", LOG_FILE_NAME)
        return
        ...
        with active_ai_sessions_lock:
            ...
            while i < len(active_ai_sessions):
                ...
                try:
                    ...
                    # Vérifie si la session est marquée comme complétée
                    if getattr(session, 'completed', False):
                        ai_session.stop()
                        active_ai_sessions.pop(i)
                        utils.log_message(f"Session complétée détectée: {getattr(session_record, 'name', 'Unknown')} (ID: {session_id})", LOG_FILE_NAME)
